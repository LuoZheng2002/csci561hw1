    ...
    pub fn remove(&mut self, target: &T) {
        ...
        for i in (-root_level..).map(|x| -x) {
            ...
            let current_cover_set = level_to_cover_set
                .get(&i)
                .expect("Cannot find the query, reached the bottom of the tree.");
    ...
    pub fn print(&self) {
        ...
        loop {
            ...
            let max_set_size = current_level_node_sets
                ...
                .max()
                .unwrap_or(0);
            let num_nodes = current_level_node_sets.iter().map(|set| set.len()).sum::<usize>();
            println!(
